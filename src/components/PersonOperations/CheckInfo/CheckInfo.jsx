...
const CheckInfo = () => {
    ...
    const searchPersonByDocument = async () => {
        ...
        try {
            const response = await crediexpressAPI.post("func/search_person", {
            ...
            switch (nextStep) {
                case "debtorRegister":
                    navigate(`/register?document=${document}`)
                break

                case "updateDebtorData":
                    // Navega al formulario con datos cargados para actualizar
                    navigate(`/register?edit=true&id=${id_person}`)
                break
        ...
        <div className="bg-[url('./assets/bg-pred-resp.png')] md:bg-[url('./assets/bg-pred.png')] bg-cover bg-center h-dvh w-full p-5">
            <div className="flex items-center justify-center h-full lg:mr-16">
                <div className="w-full ml-auto max-w-md bg-white shadow-2xl border border-gray-400 p-6 rounded-2xl">
                    ...
                    <p className="text-center mt-3">Ingresa la información  para crear al solicitante </p>
                    ...
                    <form className="space-y-5 mt-5" onSubmit={e => e.preventDefault()}>
                        ...
                        <div className="col-span-2">
                            <div className="bg-yellow-400 rounded-lg p-2 text-white font-semibold text-center cursor-pointer hover:bg-yellow-500"
